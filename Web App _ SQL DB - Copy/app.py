    ...
    if mac_address_param:
        ...
        if device:
            ...
            if latest_firmware_entry:
                ...
                filename = f"{sanitized_mac_address}_{latest_firmware_entry.file_name}"
    ...
    if 'firmware' in request.files:
        ...
        if firmware_file.filename != '':
            ...
            if version is not None:
                ...
                if device:
                    history_entry = FirmwareHistory(device_id=device.id, file_name=firmware_file.filename)
...
def delete_history_entry():
    ...
    if entry_id and mac_address:
        ...
        if history_entry:
            sanitized_mac_address = mac_address.replace(':', '_')
            filename = f"{sanitized_mac_address}_{history_entry.file_name}"
